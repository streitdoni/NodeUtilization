...
#include "../../../../../Repositories/Simulations/inetmanet-3.x/src/inet/common/InitStages.h"
#include "../../../../../Repositories/Simulations/inetmanet-3.x/src/inet/common/NotifierConsts.h"
#include "../../../../../Repositories/Simulations/inetmanet-3.x/src/inet/networklayer/common/InterfaceEntry.h"
#include "../../../../../Repositories/Simulations/inetmanet-3.x/src/inet/networklayer/common/IPProtocolId_m.h"
#include "../../../../../Repositories/Simulations/inetmanet-3.x/src/inet/networklayer/common/L3Address.h"
#include "../../../../../Repositories/Simulations/inetmanet-3.x/src/inet/networklayer/contract/INetworkProtocolControlInfo.h"
#include "../../../../../Repositories/Simulations/inetmanet-3.x/src/inet/networklayer/contract/IRoute.h"
#include "../../../../../Repositories/Simulations/inetmanet-3.x/src/inet/networklayer/ipv4/ICMPMessage.h"
...
simsignal_t AODVQoSRouting::rcvdRLSignal = registerSignal("rcvdRL");
...
double AODVQoSRouting::determineQoSRREQTreatment(double requiredBandwidth, double requiredSlotTime, simtime_t singleHopTransmissionTime)
{
    double overheadByte = 0.0;
    bool isIntermediate = false;
    int maxHops = 1;

    /****************************
     *Determining current Overhead
     *****************************/
    for (TotalNodeUtilization::iterator transmission = totalNodeUtilzation.begin(); transmission != totalNodeUtilzation.end(); transmission++)
    {
        defineBandwidthOverhead(transmission->second->transmissionOverhead.overhead);

        for (std::vector<NodeUtilization::TransmissionOverHead>::iterator overhead = transmission->second->transmissionOverhead.overhead.begin(); overhead != transmission->second->transmissionOverhead.overhead.end(); overhead++)
        {
            overheadByte += overhead->byte;
        }

        maxHops = transmission->second->transmissionOverhead.pathHops > maxHops ? transmission->second->transmissionOverhead.pathHops : maxHops;

        if (NodeUtilization::INTERMEDIATE == transmission->second->transmissionOverhead.position)
        {
            isIntermediate = true;
        }
    }
    ...
    {
        double capacityLeft = currentAvailability - (1 / theoreticalAvailableBytes * (requiredResources));
...
INetfilter::IHook::Result AODVQoSRouting::ensureRouteForDatagram(INetworkDatagram * datagram, const InterfaceEntry *interfaceEntry)
{
...
// valid route to the destination expires or is marked as invalid.
            ...
            {
                int tos = payload->getDiffServCodePoint();
...
void AODVQoSRouting::sendRREQ(AODVQoSRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    // TTL = NET_DIAMETER is used for each attempt.AODVQoSRouting::startQoSRouteDiscovery
...
void AODVQoSRouting::sendRREP(AODVQoSRREP *rrep, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ...
    sendAODVPacket(rrep, nextHop, timeToLive, delay);
}
...
AODVQoSRREQ *AODVQoSRouting::createRREQ(const L3Address& destAddr, double minAvailableBandwidth, double minAvailableSlotTime)
{
    ...
    rreqPacket->setOneHopTransmissionTime(simTime());
...
AODVQoSRREP *AODVQoSRouting::createQoSRREP(AODVQoSRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    if (getSelfIPAddress() != rrep->getOriginatorAddr())
    {
        ...
        if (originatorRoute && originatorRoute->getSource() == this)
        {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0)
            {
                ...
                totalNodeUtilzation[ { rrep->getOriginatorAddr(), rrep->getDestAddr() }] = new NodeUtilization(rrep->getTotalHopsToDest(), NodeUtilization::Position::INTERMEDIATE);
...
// check (ii)
    if (destRouteData && destRouteData->isActive() && destRouteData->hasValidDestNum() && destRouteData->getDestSeqNum() >= rreq->getDestSeqNum())
    {
        ...
        // send to the originator
        sendRREP(rrep, rreq->getOriginatorAddr(), 255, result);
        ...
        if (route->getSource() == this)
        {
            ...
            if (routeData->getLifeTime() <= simTime())
            {
                if (routeData->isActive())
                {
                    ...
                    // soft state corrcvdRREQWithKnownDestinationresponding to the route (e.g., last known hop count)
                    // will be lost.
                    routeData->setIsActive(false);
                    routeData->setLifeTime(simTime() + deletePeriod);
                }
...
AODVQoSRouting::~AODVQoSRouting()
{
    ...
    for (TotalNodeUtilization::iterator it = totalNodeUtilzation.begin(); it != totalNodeUtilzation.end(); it++)
    {
        delete it->second;
    }
