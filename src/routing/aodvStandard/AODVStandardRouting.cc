...
Define_Module(AODVStandardRouting);

simsignal_t AODVStandardRouting::rcvdRLSignal = registerSignal("rcvdRL");

void AODVStandardRouting::initialize(int stage)
{
...
void AODVStandardRouting::handleMessage(cMessage *msg)
{
    if (!isOperational)
    {
    ...
    else
    {
        ...
        {
            case RREQ:
                handleRREQ(check_and_cast<AODVStandardRREQ *>(ctrlPacket), sourceAddr, arrivalPacketTTL);
                break;

            case RREP:
                handleRREP(check_and_cast<AODVStandardRREP *>(ctrlPacket), sourceAddr);
                break;

            case RERR:
                handleRERR(check_and_cast<AODVStandardRERR *>(ctrlPacket), sourceAddr);
                break;

            case RREPACK:
                handleRREPACK(check_and_cast<AODVStandardRREPACK *>(ctrlPacket), sourceAddr);
                break;
...
void AODVStandardRouting::recordUtilization(int packetSize, Path path)
{

    TotalNodeUtilization::iterator it = totalNodeUtilzation.find(path);
    NodeUtilization::TransmissionOverHead overhead = { packetSize, simTime() };
    if (it != totalNodeUtilzation.end())
    {
        it->second->transmissionOverhead.overhead.push_back(overhead);
        defineBandwidthOverhead(it->second->transmissionOverhead.overhead);
    }
    else
    {
        std::vector<NodeUtilization::TransmissionOverHead> tmpVector;
        tmpVector.push_back(overhead);
        NodeUtilization* tmpUtilization = new NodeUtilization();
        tmpUtilization->transmissionOverhead.overhead = tmpVector;
        totalNodeUtilzation[path] = tmpUtilization;
    }

    double overallConsumingBandwidth = 0;

    for (auto it = totalNodeUtilzation.begin(); it != totalNodeUtilzation.end(); it++)
    {
        for (auto innerIt = it->second->transmissionOverhead.overhead.begin(); innerIt != it->second->transmissionOverhead.overhead.end(); innerIt++)
        {
            overallConsumingBandwidth += innerIt->byte;
        }
    }
    ////////////////////////////////////////////////
    ///////////Result Recording/////////////////////
...
{
    std::vector<NodeUtilization::TransmissionOverHead>::iterator it = transmissionOverhead.begin();

    while (it != transmissionOverhead.end())
    {
        if (simTime() >= (it->recordTime + recordTime))
...
INetfilter::IHook::Result AODVStandardRouting::ensureRouteForDatagram(INetworkDatagram *datagram)
{
    Enter_Method("datagramPreRoutingHook");
    const L3Address& destAddr = datagram->getDestinationAddress();
    const L3Address& sourceAddr = datagram->getSourceAddress();
    IPv4Datagram* payload = dynamic_cast<IPv4Datagram*>(datagram);
    int totalPacketLenByte=payload->getTotalLengthField()+36;
    int arpPktLenByte = 48;

    if(payload->getTransportProtocol() == IP_PROT_MANET)
    {
        recordUtilization(destAddr.isBroadcast() ? totalPacketLenByte:totalPacketLenByte+14+arpPktLenByte+arpPktLenByte);
    }
    else if(payload->getTransportProtocol()==IP_PROT_UDP)
    {
        if(sourceAddr.isUnspecified())
        {
            recordUtilization(totalPacketLenByte+14+8, Path(getSelfIPAddress(),destAddr));
        }
        else
        {
            recordUtilization((totalPacketLenByte+14+8), Path(sourceAddr,destAddr));
        }
    }
...
AODVStandardRouting::AODVStandardRouting()
{
}

bool AODVStandardRouting::hasOngoingRouteDiscovery(const L3Address& target)
{
...
L3Address AODVStandardRouting::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AODVStandardRouting::delayDatagram(INetworkDatagram *datagram)
{
...
void AODVStandardRouting::sendRREQ(AODVStandardRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVStandardRouting::sendRREP(AODVStandardRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
AODVStandardRREQ *AODVStandardRouting::createRREQ(const L3Address& destAddr)
{
    AODVStandardRREQ *rreqPacket = new AODVStandardRREQ("AODV-RREQ");
...
AODVStandardRREP *AODVStandardRouting::createRREP(AODVStandardRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    AODVStandardRREP *rrep = new AODVStandardRREP("AODV-RREP");
    rrep->setPacketType(RREP);
    rrep->setTotalHopsToDest(rreq->getHopCount());
...
AODVStandardRREP *AODVStandardRouting::createGratuitousRREP(AODVStandardRREQ *rreq, IRoute *originatorRoute)
{
    ASSERT(originatorRoute != nullptr);
    AODVStandardRREP *grrep = new AODVStandardRREP("AODV-GRREP");
...
void AODVStandardRouting::handleRREP(AODVStandardRREP *rrep, const L3Address& sourceAddr)
{
    ...
    if (getSelfIPAddress() != rrep->getOriginatorAddr())
    {
        ...
        if (originatorRoute && originatorRoute->getSource() == this)
        {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0)
            {
                ...
                {
                    AODVStandardRREPACK *rrepACK = createRREPACK();
                ...
                AODVStandardRREP *outgoingRREP = rrep->dup();
...
void AODVStandardRouting::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AODVStandardRouting::sendAODVPacket(AODVStandardControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void AODVStandardRouting::handleRREQ(AODVStandardRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    if (rreq->getDestAddr() == getSelfIPAddress())
    {
        ...
        // create RREP
        AODVStandardRREP *rrep = createRREP(rreq, destRoute, reverseRoute, sourceAddr);
    ...
    if (destRouteData && destRouteData->isActive() && destRouteData->hasValidDestNum() && destRouteData->getDestSeqNum() >= rreq->getDestSeqNum())
    {
        ...
        if (rreq->getGratuitousRREPFlag())
        {
            ...
            AODVStandardRREP *grrep = createGratuitousRREP(rreq, originatorRoute);
    ...
    if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0))
    {
        ...
        AODVStandardRREQ *outgoingRREQ = rreq->dup();
...
IRoute *AODVStandardRouting::createRoute(const L3Address& destAddr, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AODVStandardRouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void AODVStandardRouting::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
    ...
    std::vector<StandardUnreachableNode> unreachableNodes;
    ...
    AODVStandardRERR *rerr = createRERR(unreachableNodes);
    rerrCount++;
...
AODVStandardRERR *AODVStandardRouting::createRERR(const std::vector<StandardUnreachableNode>& unreachableNodes)
{
    AODVStandardRERR *rerr = new AODVStandardRERR("AODV-RERR");
...
void AODVStandardRouting::handleRERR(AODVStandardRERR *rerr, const L3Address& sourceAddr)
{
    ...
    std::vector<StandardUnreachableNode> unreachableNeighbors;
    ...
    if (unreachableNeighbors.size() > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0))
    {
        ...
        AODVStandardRERR *newRERR = createRERR(unreachableNeighbors);
...
bool AODVStandardRouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void AODVStandardRouting::clearState()
{
...
void AODVStandardRouting::handleWaitForRREP(WaitForRREP *rrepTimer)
{
    ...
    AODVStandardRREQ *rreq = createRREQ(destAddr);
...
void AODVStandardRouting::forwardRREP(AODVStandardRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVStandardRouting::forwardRREQ(AODVStandardRREQ *rreq, unsigned int timeToLive)
{
...
void AODVStandardRouting::completeRouteDiscovery(const L3Address& target)
{
...
void AODVStandardRouting::sendGRREP(AODVStandardRREP *grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
AODVStandardRREP *AODVStandardRouting::createHelloMessage()
{
    ...
    AODVStandardRREP *helloMessage = new AODVStandardRREP("AODV-HelloMsg");
...
void AODVStandardRouting::sendHelloMessagesIfNeeded()
{
    ...
    if (hasActiveRoute && (lastBroadcastTime == 0 || simTime() - lastBroadcastTime > helloInterval))
    {
        ...
        AODVStandardRREP *helloMessage = createHelloMessage();
...
void AODVStandardRouting::handleHelloMessage(AODVStandardRREP *helloMessage)
{
...
void AODVStandardRouting::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++)
    {
        IRoute *route = routingTable->getRoute(i);
...
void AODVStandardRouting::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result AODVStandardRouting::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, L3Address& nextHopAddress)
{
...
void AODVStandardRouting::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit)
    {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
...
void AODVStandardRouting::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool AODVStandardRouting::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this)
    {
...
AODVStandardRREPACK *AODVStandardRouting::createRREPACK()
{
    AODVStandardRREPACK *rrepACK = new AODVStandardRREPACK("AODV-RREPACK");
    rrepACK->setPacketType(RREPACK);
    rrepACK->setByteLength(2);
    return rrepACK;
}

void AODVStandardRouting::sendRREPACK(AODVStandardRREPACK *rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void AODVStandardRouting::handleRREPACK(AODVStandardRREPACK *rrepACK, const L3Address& neighborAddr)
{
...
void AODVStandardRouting::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void AODVStandardRouting::handleBlackListTimer()
{
